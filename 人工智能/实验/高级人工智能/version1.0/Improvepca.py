...
orlpath = "D:/PCA-Principal-Components-Analysis-master/FaceDB_orl"
# 读入人脸库,每个人随机选择9张作为训练集,其余构成测试集
def load_orl(k):
    '''
    对训练数据集进行数组初始化，用0填充，每张图片尺寸都定为112*92,
    现在共有40个人，每个人都选择8张，则整个训练集大小为40*8,112*92
    '''
    ...
    for i in range(40):  # 共有40个人
        ...
        for j in range(10):  # 每个人都有10张照片
            image = orlpath + '/0' + str(people_num).zfill(2) + '/' + str(sample[j]).zfill(2) + '.png'
            # 读取图片并进行矢量化
...
def face_rec():
    ...
    print('每个人选择8张照片进行训练，The classify accuracy is: %.2f%%' % (accuracy * 100))
