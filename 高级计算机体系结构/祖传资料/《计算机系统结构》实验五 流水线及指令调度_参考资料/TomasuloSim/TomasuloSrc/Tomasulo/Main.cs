...
namespace Tomasulo
{
    public partial class Main : Form
    {
        ...
        {
            FuncuinalUnitsUsage.RefreshFuncuinalUnitsUsage();
        ...
        {
            FuncuinalUnitsUsage[] Resrcs = new FuncuinalUnitsUsage[50];
            for (int i = 0; i < Resrcs.Length; i++)
            {
                Resrcs[i] = new FuncuinalUnitsUsage();
            }
        }
        ...
        // Programmer      : Liron Shamir
        // Description     : Running all flow in one single run. the grids will update simultaniesly
        // Return type     : void 
        //============================================================
        private void RunAll()
        {
            while (!this.dgvRob.Rows[dgvInstructionQueue.Rows.Count - 1].Cells["State"].Value.ToString().Equals("Commit"))
            {
        ...
        // Programmer      : Liron Shamir
        // Description     : Running one step in flow. the grids will update simultaniesly
        // Return type     : void 
        ...
        private void OneStepExecution(bool runAll)
        {
            ...
            FuncuinalUnitsUsage.Insert(numOfIterations, int.Parse(ClockLbl.Text));
        ...
        private static void ResetExecutionVars()
        {
        ...
        //============================================================
        // Function name   : InstructionStatusScroolRePosition
        // Programmer      : Liron Shamir
        // Description     : reposition the instruction status grid after the last committed instruction
        // Return type     : void 
        //============================================================
        private void InstructionStatusScroolRePosition()
        {
        ...
        private void BuildFUTimers()
        {
            ...
            for (int i = 0; i < InstructionFromInput.InstructionsFromInputDT().Rows.Count; i++)
            {
                if (InstructionFromInput.InstructionsFromInputDT().Rows[i]["Instruction Name"].ToString().Equals("MUL.D")
                    || InstructionFromInput.InstructionsFromInputDT().Rows[i]["Instruction Name"].ToString().Equals("DIV.D"))
                {
                    this.execution.AddMulTimers["FPMullTimers"].Add(i, fpMullLength);
                }
                if (InstructionFromInput.InstructionsFromInputDT().Rows[i]["Instruction Name"].ToString().Equals("ADD.D")
        ...
        {
            FuncuinalUnitsUsageForm frm = new FuncuinalUnitsUsageForm();
            frm.Show();
        }
